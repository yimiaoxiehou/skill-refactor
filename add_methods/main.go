...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "Usage: add_methods <McpHttpServer.java path>")
		os.Exit(1)
	}
	...
	oldListEnd := `"save_project","extensions_list","log"]`
	newListEnd := `"save_project","burp_version","add_issue","proxy_history_filtered","register_http_handler","remove_http_handler","register_proxy_rule","remove_proxy_rule","extensions_list","log"]`
	content = strings.Replace(content, oldListEnd, newListEnd, 1)
	...
	fmt.Println("Methods added and tool list updated.")
}
